...
SCRIPT_PATH="/etc/profile.d/java.sh"
...
if [ -z "$LINK" ]; then
    read -p 'JDK tzr.gz download link: ' LINK
fi
...
for dir in ${TEMP_DIR}work/*/ ; do
    ...
    echo "export JAVA_HOME=${INSTALLED_DIR}" >> $SCRIPT_PATH
    echo "export PATH=\$PATH:\$JAVA_HOME/bin" >> $SCRIPT_PATH
    echo "Making executable: ${SCRIPT_PATH}"
    chmod +x $SCRIPT_PATH
    echo "Sourcing: ${SCRIPT_PATH}"
    source $SCRIPT_PATH
    echo "\$JAVA_HOME (${OLD_HOME}) -> ${JAVA_HOME}"
done
